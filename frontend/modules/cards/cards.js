...
class CardsModule {
  ...
  renderTableRow(card) {
    ...
    <tr class="${daysSinceTransaction >= 3 ? 'warning-row' : ''}" data-card-id="${card.id}">
      <td title="${card.name}">${card.name}</td>
      <td><span class="table-status ${statusClass}">${statusText}</span></td>
      <td>${card.currency}</td>
      <td>${card.balance || 0} ${card.currency}</td>
      <td>${card.income_amount || 0} ${card.currency}</td>
      <td>${totalSpent} UAH</td>
      <td>${warmUp} UAH</td>
  ...
  renderCard(card) {
    ...
    const income = card.income_amount || 0;
    const daysSinceTransaction = this.calculateDaysSinceTransaction(card.last_transaction_date);
    ...
    <div class="card-item ${daysSinceTransaction >= 3 ? 'warning-card' : ''}" data-card-id="${card.id}">
      ...
      <div class="card-stats">
        ...
        <div class="stat-item">
          ...
          <div class="stat-value ${income > 0 ? 'positive' : ''}">${income} ${card.currency}</div>
        </div>
        <div class="stat-item">
          <div class="stat-label">Прогрев</div>
          <div class="stat-value ${warmUp > 0 ? 'warning' : ''}">${warmUp} UAH</div>
        </div>
      </div>
      ...
      <div class="card-body">
        <div class="card-info">
          ...
          <div class="card-info-item">
            <span class="card-info-label">Пополнение</span>
            <span class="card-info-value">${card.top_up_uah} UAH</span>
          </div>
          ` : ''}
          ${card.remaining_balance ? `
          <div class="card-info-item">
            <span class="card-info-label">Остаток</span>
            <span class="card-info-value">${card.remaining_balance} UAH</span>
          </div>
          ` : ''}
          <div class="card-info-item">
            <span class="card-info-label">Скручено</span>
            <span class="card-info-value">${totalSpent} UAH</span>
          </div>
          ${card.full_name ? `
          <div class="card-info-item">
